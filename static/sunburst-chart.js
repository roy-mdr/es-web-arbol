// Version 1.8.0 sunburst-chart - https://github.com/vasturiano/sunburst-chart
// @ts-nocheck
(function (global, factory) {
  ...
  function node_sum(value, percentage) {
    return this.eachAfter(function (node) {
      var sum = +value(node.data) || 0,
        children = node.children,
        i = children && children.length;
      while (--i >= 0) sum += children[i].value;
      var percent = percentage(node.data) || 0;
      node.value = sum * (1 + +percent);
    });
  }
  ...
  var sunburst = index({
    ...
    methods: {
      _parseData: function _parseData(state) {
        if (state.data) {
          ...
          var hierData = hierarchy(state.data, accessorFn(state.children)).sum(accessorFn(state.size), accessorFn(state.sumfactor));
    ...
    update: function update(state) {
      ...
      state.radiusScale.range([maxRadius * state.centerRadius, maxRadius]);
